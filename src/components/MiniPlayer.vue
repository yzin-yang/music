<template>
    <div class="mini-wrapper" @click="testShowPlayer">
        <img :src="playingSong.picUrl" class="mini-poster" :class="{ rotate: playState }" />
        <div class="mini-info">
        ...
        <span class="mini-buttons">
            <i
                ...
                :class="{audiozantingtingzhi: playState, audiobofang2: !playState}"
                @click.stop="toggle"
            />
            <i class="audio audioliebiao mini-button" @click.stop="showAudioList" />
        </span>
    </div>
</template>
