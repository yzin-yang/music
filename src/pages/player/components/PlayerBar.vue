...
export default {
    ...
    methods: {
        ...
        move(e) {
            ...
            //控制边界
            const progress = Math.min(Math.max(0, currentX), this.barWidth);
            this.touchWidth = (progress / this.barWidth) * 100;
            this.changeWidth(this.touchWidth);
        },
        end() {
            this.$emit('changeTime', this.touchWidth);
        },
        clickBar(e) {
            const currentX = e.changedTouches[0].pageX - this.barOffsetLeft;
            const progress = Math.min(Math.max(0, currentX), this.barWidth);
